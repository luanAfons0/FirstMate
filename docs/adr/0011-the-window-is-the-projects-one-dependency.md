...
A taskbar button is grouped by an Application User Model ID, and a program that never sets one is grouped by the executable that runs it. FirstMate is run by node.exe, so the taskbar drew Node's icon beside a window whose own icon was already FirstMate's, and setting the window icon again did not help. The window is told its own identity and where its icon is, before it is first shown, because the shell reads that when it makes the button and a button already made keeps what it read. There is no way to reach the shell's property store from Node, so this is the one thing the running program asks PowerShell to do for it, and the one place the window is made hidden and shown once it is right.
