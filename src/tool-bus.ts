 ...
 */
export function openToolBus(plugins: readonly PluginRow[], reach: Reach): ToolBus {
  ...
  return {
    answering: (from) => (method, params) =>
      carry({ registered, grants, carrying, reach }, from, method, params),
  };
}
...
async function carry(bus: Bus, from: string, method: string, params: unknown): Promise<Answer> {
  ...
  const onward: JsonRpcMessage = {
    ...
    params: without(asked, 'plugin'),
  };

  hold(bus.carrying, to, chain);
  try {
    const answered = await server.call(onward);
...
function without(params: Record<string, unknown>, key: string): Record<string, unknown> {
  const rest = { ...params };
  delete rest[key];
  return rest;
}
